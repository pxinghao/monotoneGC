...
$\tt{GCImax(A$_I$, f, B$_I$)} = \tilde{\tt{A}}$, where $\tilde{\tt{A}} \subseteq \tt{A}_I$ is a set satisfying
\begin{align}
\forall\hat{\tt{A}} \supseteq \tt{A}_I - \tilde{\tt{A}},
\quad \tt{f($\hat{\tt{A}}$)} \cup \tt{B}_I = \tt{f($\hat{\tt{A}}$ $\cup$ $\tilde{\tt{A}}$)} \cup \tt{B}_I,\label{eq:gcimax1}
\end{align}
and for all $\tt{A}_\text{TS}'$ satisfying the GC Invariant \ref{inv:gci} Eq \ref{eq:gcinv1},\label{eq:gcimax2}
\begin{align}
\forall \hat{\tt{A}} \supseteq \tt{A}_I \cup \tt{A}_\exists' - (\tilde{\tt{A}} \cup \tt{A}_\top'),
\quad \tt{f($\hat{\tt{A}}$)} \cup \tt{B}_I = \tt{f($\hat{\tt{A}}$ $\cup$ $\tilde{\tt{A}}$ $\cup$ \tt{A}$_\top'$)} \cup \tt{B}_I
\end{align}
\end{example}
...
Next, we show that $\tilde{\tt{A}} \cup \tt{A}_\top$ satisfies conditions Eq \eqref{eq:maxgc1} and \eqref{eq:maxgc2}.
...
\begin{align*}
\forall\hat{\tt{A}} \supseteq \tt{A}_I - \tilde{\tt{A}},
\quad \tt{f($\hat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\hat{\tt{A}}$ $\cup$ $\tilde{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align*}
so $(\emptyset, \tt{A}_I - \tilde{\tt{A}}, \tilde{\tt{A}})$ satisfy GC Invariant \ref{inv:gci} Eq \ref{eq:gcinv1}.
Applying Eq \ref{eq:gcinv2}, we get
\begin{align*}
&\forall \hat{\tt{A}}
  ~~\supseteq~~ \tt{A}_\exists \cup (\tt{A}_I - \tilde{\tt{A}}) - (\tt{A}_\top \cup \tilde{\tt{A}})
  ~~=~~ \tt{A}_\exists - (\tt{A}_\top \cup \tilde{\tt{A}}),\\
&\tt{f($\hat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top
  ~~=~~ \tt{f($\hat{\tt{A}}$ $\cup$ A$_\top$ $\cup$ $\tilde{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align*}
thus satisfying Eq \eqref{eq:maxgc1} with tombstones $\tilde{\tt{A}} \cup \tt{A}_\top$. Similarly, Eq \eqref{eq:gcimax2} implies
\begin{align}
\forall \hat{\tt{A}} \supseteq \tt{A}_I \cup \tt{A}_\exists' - (\tilde{\tt{A}} \cup \tt{A}_\top'),
\quad \tt{f($\hat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\hat{\tt{A}}$ $\cup$ $\tilde{\tt{A}}$ $\cup$ \tt{A}$_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align}
so $(\emptyset, \tt{A}_I \cup \tt{A}_\exists' - (\tilde{\tt{A}} \cup \tt{A}_\top'), \tilde{\tt{A}} \cup \tt{A}_\top')$ satisfy GC Invariant \ref{inv:gci} Eq \ref{eq:gcinv1}.
Applying Eq \ref{eq:gcinv2}, we get
\begin{align*}
&\forall \hat{\tt{A}}
  ~~\supseteq~~ \tt{A}_\exists \cup (\tt{A}_I \cup \tt{A}_\exists' - (\tilde{\tt{A}} \cup \tt{A}_\top')) - (\tt{A}_\top \cup \tilde{\tt{A}} \cup \tt{A}_\top')
  ~~=~~ \tt{A}_\exists \cup \tt{A}_\exists' - (\tt{A}_\top \cup \tilde{\tt{A}} \cup \tt{A}_\top'),\\
&\tt{f($\hat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\hat{\tt{A}}$ $\cup$ A$_\top$ $\cup$ $\tilde{\tt{A}}$ $\cup$ \tt{A}$_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top
\end{align*}
thus satisfying Eq \eqref{eq:maxgc2} with tombstones $\tilde{\tt{A}} \cup \tt{A}_\top$.
We have therefore shown that $\tt{GCImax(A$_I$, f, B$_I$)} = \tilde{\tt{A}} \subseteq \tilde{\tt{A}} \cup \tt{A}_\top \subseteq \tt{GCmax(A$_\text{TS}$, f, B$_\text{TS}$)}$ due to the maximality of $\tt{GCmax}$.
