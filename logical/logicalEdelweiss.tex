...
\begin{align}
\forall i,~~ \forall\widehat{\tt{A}} \supseteq \tt{A}_\exists,
\quad \tt{f$_i$($\widehat{\tt{A}}$)} \cup \tt{B}_{i,\exists} \cup \tt{B}_{i,\top} = \tt{f$_i$($\widehat{\tt{A}} \cup \tt{A}_\top$)} \cup \tt{B}_{i,\exists} \cup \tt{B}_{i,\top} \label{eq:gcinv1}
\end{align}
Furthermore, for $\tt{A}_\text{TS}$ and $\tt{A}_\text{TS}'$ satisfying Eq \eqref{eq:gcinv1}, it must be that
\begin{align}
\forall i,~~ \forall\widehat{\tt{A}} \supseteq \tt{A}_\exists \cup \tt{A}_\exists' - (\tt{A}_\top \cup \tt{A}_\top'),
\quad \tt{f$_i$($\widehat{\tt{A}}$)} \cup \tt{B}_{i,\exists} \cup \tt{B}_{i,\top} = \tt{f$_i$($\widehat{\tt{A}} \cup \tt{A}_\top \cup \tt{A}_\top'$)} \cup \tt{B}_{i,\exists} \cup \tt{B}_{i,\top} \label{eq:gcinv2}
\end{align}
\end{invariant}
...
% \begin{lemma}
% The GC Rule Conservation Property \ref{prp:gcrconserve} implies GC Rule Monotonicity Property \ref{prp:gcrmonotone}.
% \end{lemma}

% \begin{lemma}
% Suppose the $\tt{GC}$ rule is monotone, conservative, and maintains GC Invariant \ref{inv:gci} Eq \eqref{eq:gcinv1}. Then \tt{GC} satisfies GC Invariant \ref{inv:gci} Eq \eqref{eq:gcinv2}.
% \end{lemma}
% \begin{proof}
% Let $\tt{X} = \tt{A}_\text{TS} \sqcup \tt{A}_\text{TS}'$ where $\tt{A}_\text{TS}$ and $\tt{A}_\text{TS}$ both satisfy GC Invariant \ref{inv:gci} Eq \eqref{eq:gcinv1}.
% Let $\tt{G} = \tt{GC}(\tt{X}, \tt{f}, \tt{B}_\text{TS})$.
% By conservation, $\tt{G} \geq \tt{X}$, and by monotonicity, $\tt{X} \geq \tt{A}_\text{TS}$, $\tt{X} \geq \tt{A}_\text{TS}'$.
% This implies $\tt{G}_\top \geq \tt{A}_\text{TS} \cup \tt{A}_\text{TS}'$, and therefore by conservation $\tt{G}_\exists = \tt{A}_\exists \cup \tt{A}_\exists' - (\tt{A}_\top \cup \tt{A}_\top')$.
% \end{proof}
...
Suppose also $\tt{A}_\text{TS}'$ satisfies the GC Invariant \ref{inv:gci} with respect to $\tt{B}_{i,\text{TS}}' \geq \tt{B}_{i,\text{TS}}$.
Then
\begin{align*}
\tt{A}_\top \cup \left(\tt{A}_\exists \cap \bigcap_{i=1}^k (\tt{B}_{i,\exists} \cup \tt{B}_{i,\top})\right)
&\subseteq \tt{A}_\top' \cup \left((\tt{A}_\exists' \cup \tt{A}_\top') \cap \bigcap_{i=1}^k (\tt{B}_{i,\exists}' \cup \tt{B}_{i,\top}')\right)\\
&= \tt{A}_\top' \cup \left(\tt{A}_\exists' \cap \bigcap_{i=1}^k (\tt{B}_{i,\exists}' \cup \tt{B}_{i,\top}')\right) \cup \left(\tt{A}_\top' \cap \bigcap_{i=1}^k (\tt{B}_{i,\exists}' \cup \tt{B}_{i,\top}')\right)\\
&= \tt{A}_\top' \cup \left(\tt{A}_\exists' \cap \bigcap_{i=1}^k (\tt{B}_{i,\exists}' \cup \tt{B}_{i,\top}')\right),
\end{align*}
where the final equality follows because $\forall i=1,\dots, k, ~\tt{A}_\top' \subseteq \tt{B}_{i,\exists}' \cup \tt{B}_{i,\top}'$.
Furthermore, it follows from the GC Rule Conservation Property \ref{prp:gcrconserve}, we also have that $(\tt{A}_\exists - \tt{B}_\cap) \cup (\tt{A}_\top \cup \tt{B}_\cap) \subseteq (\tt{A}_\exists' - \tt{B}_\cap') \cup (\tt{A}_\top' \cup \tt{B}_\cap')$.
Hence, $\tt{GCcopy}$ is monotone.

\begin{example}[Max GC]
$\tt{GCmax(A$_\text{TS}$, f, B$_\text{TS}$)} = (\emptyset, \tt{A}_\exists - \widetilde{\tt{A}}, \widetilde{\tt{A}})$ where $\tt{A}_\top \subseteq \widetilde{\tt{A}} \subseteq \tt{A}_\exists \cup \tt{A}_\top$ is the (unique) largest set such that
\begin{align}
\forall\widehat{\tt{A}} \supseteq \tt{A}_\exists - \widetilde{\tt{A}}, \quad \tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\widehat{\tt{A}} \cup \widetilde{\tt{A}} \cup \tt{A}_\top$)} \cup \tt{B}_\exists \cup \tt{B}_\top,\label{eq:maxgc1}
\end{align}
and for any $\tt{A}_\text{TS}'$ satisfying the GC Invariant \ref{inv:gci} Eq \eqref{eq:gcinv1},
\begin{align}
\forall\widehat{\tt{A}} \supseteq \tt{A}_\exists \cup \tt{A}_\exists' - ({\widetilde{\tt{A}} \cup \tt{A}_\top} \cup \tt{A}_\top'), \quad \tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\widehat{\tt{A}} \cup \widetilde{\tt{A}} \cup \tt{A}_\top \cup \tt{A}_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top.\label{eq:maxgc2}
\end{align}
\end{example}
...
&\forall\widehat{\tt{A}}
  ...
  ~~=~~ \tt{A}_\exists - \tt{Z},\\
&\tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \tt{X} \cup \tt{A}_\top$)} \cup \tt{B}_\exists \cup \tt{B}_\top
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \tt{X} \cup \tt{Y} \cup \tt{A}_\top \cup \tt{A}_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align*}
where we have used the fact that $\widehat{\tt{A}} \cup \tt{X} \cup \tt{A}_\top \cup \tt{A}_\top' \supseteq \tt{A}_\exists - \tt{Y}$ and Eq \eqref{eq:maxgc1} for $\tt{Y}$.
...
&\tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \tt{X} \cup \tt{A}_\top \cup \tt{A}_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align*}
...
&\tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \tt{Y} \cup \tt{A}_\top \cup \tt{A}_\top''$)} \cup \tt{B}_\exists \cup \tt{B}_\top
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \tt{X} \cup \tt{Y} \cup \tt{A}_\top \cup \tt{A}_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top.
\end{align*}
...
Similarly, the GC Rule Conservation Property \ref{prp:gcrconserve} is also maintained by the choice of $\tt{A}_\top \subseteq \widetilde{\tt{A}} \subseteq \tt{A}_\exists \cup \tt{A}_\top$.
...
For the $\tt{A}$ argument, we observe that $\widetilde{\tt{A}} \cup \tt{A}_\top'$ satisfies conditions Eq \eqref{eq:maxgc1} and Eq \eqref{eq:maxgc2} as applied to $\tt{A}_\text{TS}'$.
For condition Eq \eqref{eq:maxgc1},
\begin{align*}
&\forall \widehat{\tt{A}}
  \supseteq \tt{A}_\exists' - (\widetilde{\tt{A}} \cup \tt{A}_\top')
  ~~=~~ (\tt{A}_\exists' \cup (\tt{A}_\exists - \tt{A}_\top')) - (\widetilde{\tt{A}} \cup \tt{A}_\top \cup \tt{A}_\top')
  ~~=~~ \tt{A}_\exists' \cup \tt{A}_\exists - (\widetilde{\tt{A}} \cup \tt{A}_\top \cup \tt{A}_\top'),\\
&f(\widehat{\tt{A}}) \cup \tt{B}_\exists' \cup \tt{B}_\top'
  ~~=~~ f(\widehat{\tt{A}} \cup \widetilde{\tt{A}} \cup \tt{A}_\top') \cup \tt{B}_\exists' \cup \tt{B}_\top',
\end{align*}
...
\begin{align*}
&\forall\widehat{\tt{A}}
  ~~\supseteq~~ \tt{A}_\exists \cup \tt{A}_\exists'' - (\widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top \cup \tt{A}_\top'')
  ~~=~~ \tt{A}_\exists \cup \tt{A}_\exists'' - (\widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top''),\\
&\tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists' \cup \tt{B}_\top'
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top \cup \tt{A}_\top''$)} \cup \tt{B}_\exists' \cup \tt{B}_\top'
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top''$)} \cup \tt{B}_\exists' \cup \tt{B}_\top',
\end{align*}
so $(\emptyset, \tt{A}_\exists \cup \tt{A}_\exists'' - (\widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top''), \widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top'')$ satisfies Eq \eqref{eq:gcinv1}.
Repeating the process for $\tt{A}_\text{TS}'$,
\begin{align*}
&\forall \widehat{\tt{A}}
  ~~\supseteq~~ \tt{A}_\exists' \cup (\tt{A}_\exists \cup \tt{A}_\exists'' - (\widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top'')) - (\widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top'' \cup \tt{A}_\top'')
  ~~=~~ \tt{A}_\exists' \cup \tt{A}_\exists'' - (\widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top''),\\
&\tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists' \cup \tt{B}_\top'
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \tt{A}_\top' \cup \widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top''$)} \cup \tt{B}_\exists' \cup \tt{B}_\top'
  ~~=~~ \tt{f($\widehat{\tt{A}} \cup \widetilde{\tt{A}} \cup \tt{A}_\top' \cup \tt{A}_\top''$)} \cup \tt{B}_\exists' \cup \tt{B}_\top',
\end{align*}
...
\begin{example}[Instantiated Copy GC]
$\tt{GCIcopy(A$_I$, Id, B$_{1,I}$, $\dots$, B$_{k,I}$)} = \tt{A}_I \cap \bigcap_{i=1}^k \tt{B}_{i,I}$.
\end{example}
The $\tt{GCcopy}$ rule creates tombstones for $\tt{A}_\top \cup \left(\tt{A}_\exists \cap \bigcap_{i=1}^k (\tt{B}_{i,\exists} \cup \tt{B}_{i,\top})\right)$.
From the Representation Invariant \ref{inv:representation}, we know $\tt{A}_\exists \subseteq \tt{A}_I \subseteq \tt{A}_\exists \cup \tt{A}_\top$ and $\tt{B}_\exists \subseteq \tt{B}_I \subseteq \tt{B}_\exists \cup \tt{B}_\top$.
Thus,
\begin{align*}
\tt{A}_I \cap \bigcap_{i=1}^k \tt{B}_{i,I}
~\subseteq~ (\tt{A} \cup \tt{A}_\top) \cap \bigcap_{i=1}^k (\tt{B}_\exists \cup \tt{B}_\top)
~\subseteq~ (\tt{A} \cup \tt{A}_\top) \cap \left(\tt{A}_\top \cup \bigcap_{i=1}^k (\tt{B}_\exists \cup \tt{B}_\top)\right)
~=~ \tt{A}_\top \cup \left(\tt{A}_\exists \cap \bigcap_{i=1}^k (\tt{B}_{i,\exists} \cup \tt{B}_{i,\top})\right),
\end{align*}
so $\tt{GCIcopy}$ satisfies the Instantiated GC Safe Deletion Property \ref{prp:gcisafe}.

\begin{example}[Instantiated Max GC]
$\tt{GCImax(A$_I$, f, B$_I$)} = \widetilde{\tt{A}}$, where $\widetilde{\tt{A}} \subseteq \tt{A}_I$ is a set satisfying
\begin{align}
\forall\widehat{\tt{A}} \supseteq \tt{A}_I - \widetilde{\tt{A}},
\quad \tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_I = \tt{f($\widehat{\tt{A}}$ $\cup$ $\widetilde{\tt{A}}$)} \cup \tt{B}_I,\label{eq:gcimax1}
\end{align}
and for all $\tt{A}_\text{TS}'$ satisfying the GC Invariant \ref{inv:gci} Eq \ref{eq:gcinv1},\label{eq:gcimax2}
\begin{align}
\forall \widehat{\tt{A}} \supseteq \tt{A}_I \cup \tt{A}_\exists' - (\widetilde{\tt{A}} \cup \tt{A}_\top'),
\quad \tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_I = \tt{f($\widehat{\tt{A}}$ $\cup$ $\widetilde{\tt{A}}$ $\cup$ \tt{A}$_\top'$)} \cup \tt{B}_I
\end{align}
\end{example}
We first point out that the $\tt{GCImax}$ rule does not require knowledge of the value of any actual $\tt{A}_\text{TS}'$, as it has to hold true for \textit{all} such sets.

Next, we show that $\widetilde{\tt{A}} \cup \tt{A}_\top$ satisfies conditions Eq \eqref{eq:maxgc1} and \eqref{eq:maxgc2}.
The first condition Eq \eqref{eq:gcimax1}, together with the fact that $\tt{B}_I \subseteq \tt{B}_\top \cup \tt{B}_\exists$ implies that
\begin{align*}
\forall\widehat{\tt{A}} \supseteq \tt{A}_I - \widetilde{\tt{A}},
\quad \tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\widehat{\tt{A}}$ $\cup$ $\widetilde{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align*}
so $(\emptyset, \tt{A}_I - \widetilde{\tt{A}}, \widetilde{\tt{A}})$ satisfy GC Invariant \ref{inv:gci} Eq \ref{eq:gcinv1}.
Applying Eq \ref{eq:gcinv2}, we get
\begin{align*}
&\forall \widehat{\tt{A}}
  ~~\supseteq~~ \tt{A}_\exists \cup (\tt{A}_I - \widetilde{\tt{A}}) - (\tt{A}_\top \cup \widetilde{\tt{A}})
  ~~=~~ \tt{A}_\exists - (\tt{A}_\top \cup \widetilde{\tt{A}}),\\
&\tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top
  ~~=~~ \tt{f($\widehat{\tt{A}}$ $\cup$ A$_\top$ $\cup$ $\widetilde{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align*}
thus satisfying Eq \eqref{eq:maxgc1} with tombstones $\widetilde{\tt{A}} \cup \tt{A}_\top$. Similarly, Eq \eqref{eq:gcimax2} implies
\begin{align}
\forall \widehat{\tt{A}} \supseteq \tt{A}_I \cup \tt{A}_\exists' - (\widetilde{\tt{A}} \cup \tt{A}_\top'),
\quad \tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\widehat{\tt{A}}$ $\cup$ $\widetilde{\tt{A}}$ $\cup$ \tt{A}$_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top,
\end{align}
so $(\emptyset, \tt{A}_I \cup \tt{A}_\exists' - (\widetilde{\tt{A}} \cup \tt{A}_\top'), \widetilde{\tt{A}} \cup \tt{A}_\top')$ satisfy GC Invariant \ref{inv:gci} Eq \ref{eq:gcinv1}.
Applying Eq \ref{eq:gcinv2}, we get
\begin{align*}
&\forall \widehat{\tt{A}}
  ~~\supseteq~~ \tt{A}_\exists \cup (\tt{A}_I \cup \tt{A}_\exists' - (\widetilde{\tt{A}} \cup \tt{A}_\top')) - (\tt{A}_\top \cup \widetilde{\tt{A}} \cup \tt{A}_\top')
  ~~=~~ \tt{A}_\exists \cup \tt{A}_\exists' - (\tt{A}_\top \cup \widetilde{\tt{A}} \cup \tt{A}_\top'),\\
&\tt{f($\widehat{\tt{A}}$)} \cup \tt{B}_\exists \cup \tt{B}_\top = \tt{f($\widehat{\tt{A}}$ $\cup$ A$_\top$ $\cup$ $\widetilde{\tt{A}}$ $\cup$ \tt{A}$_\top'$)} \cup \tt{B}_\exists \cup \tt{B}_\top
\end{align*}
thus satisfying Eq \eqref{eq:maxgc2} with tombstones $\widetilde{\tt{A}} \cup \tt{A}_\top$.
We have therefore shown that $\tt{GCImax(A$_I$, f, B$_I$)} = \widetilde{\tt{A}} \subseteq \widetilde{\tt{A}} \cup \tt{A}_\top \subseteq \tt{GCmax(A$_\text{TS}$, f, B$_\text{TS}$)}$ due to the maximality of $\tt{GCmax}$.
...
\section{Concrete Rules}

\subsection{DR+}
The DR+ technique reclaims storage whenever a set difference is observed in the Bloom program.
Specifically, we identify patterns of the form
\begin{align*}
\tt{Z <= X.notin(Y, :kx => :ky)}
\end{align*}
Letting $\widetilde{\tt{X}} = ((\tt{X}_\exists \cup \tt{X}_\top) \ast (\tt{Y}_\exists \cup \tt{Y}_\top))\tt{.lefts}(\tt{:kx => :ky})$,
the logical reclamation rule is
\begin{align}
\tt{Z}_\text{TS} &\tt{ <= } (\tt{X}_\exists \cup \tt{X}_\top)\tt{.notin}(\tt{Y}_\exists \cup \tt{Y}_\top, \tt{:kx => :ky})\\
\tt{X}_\text{TS} &\tt{ <= } (\emptyset, \tt{X}_\exists - \widetilde{\tt{X}}, \tt{X}_\top \cup \widetilde{\tt{X}}) \label{eq:drpluslogical}
\end{align}
and the instantiated reclamation rule is
\begin{align}
\tt{Z}_I &\tt{ <= } \tt{X}_I\tt{.notin}(\tt{Y}_I, \tt{:kx => :ky})\\
\tt{X}_I &\tt{ <\# } (\tt{X}_I \ast \tt{Y}_I)\tt{.lefts}(\tt{:kx -> :ky}). \label{eq:drplusinstantiated}
\end{align}

It is easy to show that the logical rule Eq \eqref{eq:drpluslogical} is monotone and conservative.
We also claim that the GC Invariant \ref{inv:gci} Eq \eqref{eq:gcinv1} reduces to
\begin{align*}
\tt{X}_\top\tt{.notin}(\tt{Y}_\exists \cup \tt{Y}_\top, \tt{:kx => :ky}) = \emptyset,
\end{align*}
and GC Invariant \ref{inv:gci} Eq \eqref{eq:gcinv2} reduces to if $(\tt{X}_\text{TS}, \tt{Y}_\text{TS})$ and $(\tt{X}_\text{TS}', \tt{Y}_\text{TS}')$ satisfy Eq \eqref{eq:gcimax1}, then
\begin{align*}
(\tt{X}_\top \cup \tt{X}_\top')\tt{.notin}(\tt{Y}_\exists \cup \tt{Y}_\top \cup \tt{Y}_\exists' \cup \tt{Y}_\top', \tt{:kx => :ky}) = \emptyset.
\end{align*}

We now show that the logical $\tt{GC}$ rule satisfies Eq \eqref{eq:maxgc1} and Eq \eqref{eq:maxgc2}:
\begin{align*}
&\tt{X}_\top^t\tt{.notin}(\tt{Y}_\exists^t \cup \tt{Y}_\top^t, \tt{:kx => :ky})\\
&= (\tt{X}_\top^{t-1} \cup \widetilde{\tt{X}})\tt{.notin}(\tt{Y}_\exists^{t-1} \cup \tt{Y}_\top^{t-1}, \tt{:kx => :ky})\\
&= \widetilde{\tt{X}}\tt{.notin}(\tt{Y}_\exists^{t-1} \cup \tt{Y}_\top^{t-1}, \tt{:kx => :ky})\\
&= ((\tt{X}_\exists^{t-1} \cup \tt{X}_\top^{t-1}) \ast (\tt{Y}_\exists^{t-1} \cup \tt{Y}_\top^{t-1]}))\tt{.lefts}(\tt{:kx => :ky})\tt{.notin}(\tt{Y}_\exists^{t-1} \cup \tt{Y}_\top^{t-1}, \tt{:kx => :ky})\\
&= \emptyset,
\end{align*}
\begin{align*}
&(\tt{X}_\top^t \cup \tt{X}_\top'^t)\tt{.notin}(\tt{Y}_\exists^t \cup \tt{Y}_\top^t \cup \tt{Y}_\exists'^t \cup \tt{Y}_\top'^t, \tt{:kx => :ky})\\
&\subseteq \tt{X}_\top^t \tt{.notin}(\tt{Y}_\exists^t \cup \tt{Y}_\top^t, \tt{:kx => :ky}) ~~\cup~~ \tt{X}_\top'^t\tt{.notin}(\tt{Y}_\exists'^t \cup \tt{Y}_\top'^t, \tt{:kx => :ky})\\
&= \emptyset.
\end{align*}

Furthermore, we can show that the DR+ rule is a max GC rule.
For, AFSOC that some $\widetilde{\tt{W}} \not\subseteq \widetilde{\tt{X}}$, then there is some $w \in \widetilde{\tt{W}} \subseteq (\tt{X}_\exists \cup \tt{X}_\top)$ such that $w \not\in \widetilde{\tt{X}}$ but also $\{w\}\tt{.notin}(\tt{Y}_\exists \cup \tt{Y}_\top, \tt{:kx => :ky}) = \emptyset$.
But that would imply $w \in ((\tt{X}_\exists \cup \tt{X}_\top) \ast (\tt{Y}_\exists \cup \tt{Y}_\top))\tt{.lefts}(\tt{:kx => :ky}) = \widetilde{\tt{X}}$.

It remains to show that the instantiated GC rule satisfies the Instantiated GC Safe Deletion Property \ref{prp:gcisafe}, which follows straightforwardly from the fact that $\tt{X}_I \subseteq \tt{X}_\exists \cup \tt{X}_\top$ and $\tt{Y}_I \subseteq \tt{Y}_\exists \cup \tt{Y}_\top$:
\begin{align*}
(\tt{X}_I \ast \tt{Y}_I)\tt{.lefts}(\tt{:kx => :ky})
&\subseteq ((\tt{X}_\exists \cup \tt{X}_\top) \ast (\tt{Y}_\exists \cup \tt{Y}_\top))\tt{.lefts}(\tt{:kx => :ky})
\end{align*}
...
% \subsection{DR-}
% DR- is also a GC rule for the $\tt{notin}$ pattern $\tt{Z} \tt{ <= } \tt{X}\tt{.notin}(\tt{Y}, \tt{:kx => :ky})$, but where $\tt{kx}$ is the primary key of $\tt{X}$.
% This allows for an opportunity to reclaim from $\tt{Y}$ as well: once a tuple in $\tt{X}$ is tombstoned, we can also tombstone its counterpart in $\tt{Y}$.

% The logical reclamation rules are
% \begin{align*}
% \tt{X}_\text{TS} &\tt{ <= } (\emptyset, \tt{X}_\exists - \widetilde{\tt{X}}, \tt{X}_\top \cup \widetilde{\tt{X}})\\
% \tt{Y}_\text{TS} &\tt{ <= } (\emptyset, \tt{Y}_\exists - \widetilde{\tt{Y}}, \tt{Y}_\top \cup \widetilde{\tt{Y}})
% \end{align*}
% where
% \begin{align*}
% \widetilde{\tt{X}} &= ((\tt{X}_\exists \cup \tt{X}_\top) \ast (\tt{Y}_\exists \cup \tt{Y}_\top))\tt{.lefts}(\tt{:kx => :ky}),\\
% \widetilde{\tt{Y}} &= (\tt{X}_\top \ast (\tt{Y}_\exists \cup \tt{Y}_\top))\tt{.rights}(\tt{:kx => :ky}).
% \end{align*}
% We use the augmented representation for $\tt{X}$, which allows us to identify keys of tombstoned $\tt{X}$ tuples.
% The corresponding instantiated GC rules are
% \begin{align*}
% \tt{GCX}(\tt{X}_+, \tt{Y}_I, \tt{f}, \tt{Z}_I) &= (\tt{X}_I \ast \tt{Y}_I)\tt{.lefts}(\tt{:kx => :ky})\\
% \tt{GCY}(\tt{X}_+, \tt{Y}_I, \tt{f}, \tt{Z}_I) &= (\tt{Y}_I \ast (\tt{X.k}\tt{.notin}(\tt{X}_I, \tt{:kx => :kx})))\tt{.lefts}(\tt{:kx => :ky})
% \end{align*}

% Since the reclamation rules for $\tt{X}$ are essentially the same as for DR+, we will not repeat the analysis here.

% We now show that the logical GC rule for $\tt{Y}$ is a specific case of the $\tt{GCmax}$ rule.
% First, consider any $\widehat{\tt{X}} \supseteq \tt{X}_\exists$ and $\widehat{\tt{Y}} \supseteq \tt{Y}_\exists - \widetilde{\tt{Y}}$.
% Then,
% \begin{align*}
% \widehat{\tt{X}}\tt{.notin}(\widehat{\tt{Y}})
% &=
% \\&= \widehat{\tt{X}}\tt{.notin}(\widehat{\tt{Y}} \cup \tt{Y}_\top \cup \widetilde{\tt{Y}})\\
% &= (\widehat{\tt{X}} \cup \tt{X}_\top)\tt{.notin}(\widehat{\tt{Y}} \cup \tt{Y}_\top \cup \widetilde{\tt{Y}}) & \text{(GC Invariant on \tt{X})}\\
% \end{align*}
